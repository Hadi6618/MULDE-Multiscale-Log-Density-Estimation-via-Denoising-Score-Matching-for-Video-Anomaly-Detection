...
def smooth_scores(scores, video_lengths, sigma):
    if sigma is None or sigma <= 0:
        return scores
    scores_np = np.asarray(scores)
    smoothed = np.zeros_like(scores_np)
    start_idx = 0
    for length in video_lengths:
        end_idx = start_idx + length
        smoothed[start_idx:end_idx] = scipy.ndimage.gaussian_filter1d(scores_np[start_idx:end_idx], sigma=sigma)
        start_idx = end_idx
    return smoothed
...
def train_and_evaluate(args):
    ...
    data_train, labels_train, data_test, labels_test, id_to_type, video_lengths = get_dataset(
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % args.eval_freq == 0 or epoch == args.epochs:
            ...
            for score_type_ in anomaly_score_names:
                ...
                # Max
                max_raw = multiscale_data_test_standardized.max(axis=1)
                max_smoothed = smooth_scores(max_raw, video_lengths, args.smoothing_sigma)
                auc_roc_aggregate[f"{score_type_}"]["max"] = roc_auc_score(labels_test, max_raw)
                auc_roc_aggregate[f"{score_type_}"]["max_smoothed"] = roc_auc_score(labels_test, max_smoothed)

                # Median
                med_raw = np.median(multiscale_data_test_standardized, axis=1)
                med_smoothed = smooth_scores(med_raw, video_lengths, args.smoothing_sigma)
                auc_roc_aggregate[f"{score_type_}"]["median"] = roc_auc_score(labels_test, med_raw)
                auc_roc_aggregate[f"{score_type_}"]["median_smoothed"] = roc_auc_score(labels_test, med_smoothed)

                # Mean
                mean_raw = multiscale_data_test_standardized.mean(axis=1)
                mean_smoothed = smooth_scores(mean_raw, video_lengths, args.smoothing_sigma)
                auc_roc_aggregate[f"{score_type_}"]["mean"] = roc_auc_score(labels_test, mean_raw)
                auc_roc_aggregate[f"{score_type_}"]["mean_smoothed"] = roc_auc_score(labels_test, mean_smoothed)

                # GMM fit - takes a while:
                if args.gmm:
                    # for components_ in [1, 3, 5, 7, 9]:
                    for components_ in [1, 3, 5]:
                        # fit L-dimensional TRAIN feature vectors with GMM (with convergence warning fixes)
                        gmm = mixture.GaussianMixture(n_components=components_, covariance_type='full', max_iter=500, n_init=3).fit(multiscale_data_train_)
                        ...
                        # add smoothed NLL of GMM
                        ll_scores_smoothed = smooth_scores(-ll_scores, video_lengths, args.smoothing_sigma)
                        auc_roc_aggregate[f"{score_type_}"][f"gmm({components_})_nll_smoothed"] = roc_auc_score(labels_test, ll_scores_smoothed)
            ...
            best_auc_roc_log_density_smoothed = 0
            best_sigma_log_density_smoothed = 0
            best_auc_roc_score_norm_smoothed = 0
            best_sigma_score_norm_smoothed = 0

            all_auc_roc_log_density = list()
            all_auc_roc_score_norm = list()
            all_auc_roc_log_density_smoothed = list()
            all_auc_roc_score_norm_smoothed = list()
            ...
            for sigma in list(sorted(scores_test.keys())):
                ...
                log_densities_smoothed = smooth_scores(log_densities, video_lengths, args.smoothing_sigma)
                score_norms_smoothed = smooth_scores(score_norms_, video_lengths, args.smoothing_sigma)

                auc_roc_log_density_smoothed = roc_auc_score(labels_test, log_densities_smoothed)
                auc_roc_score_norm_smoothed = roc_auc_score(labels_test, score_norms_smoothed)

                all_auc_roc_log_density.append(auc_roc_log_density)
                all_auc_roc_score_norm.append(auc_roc_score_norm)
                all_auc_roc_log_density_smoothed.append(auc_roc_log_density_smoothed)
                all_auc_roc_score_norm_smoothed.append(auc_roc_score_norm_smoothed)
                ...
                if auc_roc_log_density_smoothed > best_auc_roc_log_density_smoothed:
                    best_auc_roc_log_density_smoothed = auc_roc_log_density_smoothed
                    best_sigma_log_density_smoothed = sigma
                if auc_roc_score_norm_smoothed > best_auc_roc_score_norm_smoothed:
                    best_auc_roc_score_norm_smoothed = auc_roc_score_norm_smoothed
                    best_sigma_score_norm_smoothed = sigma
                ...
                summary_writer.add_scalar(f"roc_auc_log_density_individual_smoothed/sigma_{sigma}", auc_roc_log_density_smoothed, epoch)
                summary_writer.add_scalar(f"roc_auc_score_norm_individual_smoothed/sigma_{sigma}", auc_roc_score_norm_smoothed, epoch)
            ...
            summary_writer.add_scalar(f"_roc_auc_best/_best_log_density_individual_smoothed", best_auc_roc_log_density_smoothed, epoch)
            summary_writer.add_scalar(f"_roc_auc_best/_best_score_norm_individual_smoothed", best_auc_roc_score_norm_smoothed, epoch)
            ...
            ax.plot(list(sorted(scores_test.keys())), all_auc_roc_log_density_smoothed, label="log density (smoothed)", linestyle="--")
            ax.plot(list(sorted(scores_test.keys())), all_auc_roc_score_norm_smoothed, label="score norm (smoothed)", linestyle="--")
            ax.legend()
            ax.set_xlabel("Sigma")
            ax.set_ylabel("AUC-ROC")
            ...
            print(f"{sep}")
            print(f"  Best Individual Log-Density AUC: {best_auc_roc_log_density:.4f} (sigma={best_sigma_log_density}) [Smoothed: {best_auc_roc_log_density_smoothed:.4f} (sigma={best_sigma_log_density_smoothed})]")
            print(f"  Best Individual Score-Norm AUC:  {best_auc_roc_score_norm:.4f} (sigma={best_sigma_score_norm}) [Smoothed: {best_auc_roc_score_norm_smoothed:.4f} (sigma={best_sigma_score_norm_smoothed})]")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--smoothing_sigma', type=float, default=3.0, help="standard deviation for 1D Gaussian temporal smoothing of test anomaly scores")
